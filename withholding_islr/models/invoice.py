...
class AccountMoveLine(models.Model):
    ...
    def islr_wh_change_concept(self, ids):
        ...
        if not ail_brw.invoice_id.state == 'open':
            raise UserError('Warning! \n Este botón debe ser usado en facturas en estado "Abierto"')
...
class AccountMove(models.Model):
    ...
    def _create_islr_wh_doc(self):
        ...
        if self.move_type in ('out_refund', 'in_refund'):
            return False
        if not (self.move_type in ('out_invoice', 'in_invoice') and rp_obj._find_accounting_partner(self.company_id.partner_id).islr_withholding_agent):
            return True
    ...
    def validate_wh_income_done(self):
        ...
        for inv in self.browse():
            if inv.move_type in ('out_invoice', 'out_refund') \
                ...
                if not rislr:
                    raise UserError("Error! \nThe Document you are trying to refund has a income withholding %s which is not yet validated!" % (inv.islr_wh_doc_id.code))
        return True
    @api.model
    def _get_move_lines2(self,
        ...
        for iwdl_brw in to_wh:
            ...
            if not acc:
                raise UserError("Falta la cuenta en el impuesto! \nEl diario de [%s] tiene las cuentas faltantes. Por favor, rellene los campos que faltan para poder continuar" % (iwdl_brw.islr_wh_doc_id.journal_id.name))
